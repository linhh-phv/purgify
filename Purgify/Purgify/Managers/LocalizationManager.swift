...
class LocalizationManager: ObservableObject {
    ...
    private let strings: [AppLanguage: [String: String]] = [
        ...
        .en: [
            ...
            "cache.rnBuild": "React Native Android Builds",
            "cache.rnBuild.detail":
                "All Android build artifacts found per React Native project, aggregated into one entry:\n\n" +
                "• android/app/build, android/build, android/.gradle\n" +
                "• node_modules/<lib>/android/build (native library outputs like libreanimated.so)\n" +
                "• node_modules/<lib>/android/.cxx (CMake/NDK cache)\n\n" +
                "Your source code is untouched. Cleaning triggers a full Android rebuild on the next run — expect a slower first build.",

            "cache.rustProject": "Rust Project target/",
            "cache.rustProject.detail":
                "The target/ folder generated by Cargo inside each Rust project (Cargo.toml).\n\n" +
                "Often the largest dev cache on disk — multi-GB per project is normal because target/ holds debug + release builds and every transitive dependency's compiled artifacts.\n\n" +
                "Your source code is untouched. Cargo recompiles dependencies on the next build (slower first run, then incremental again).",

            "cache.flutterProject": "Flutter Project Builds",
            "cache.flutterProject.detail":
                "All build artifacts found inside each Flutter project (pubspec.yaml):\n\n" +
                "• .dart_tool/, build/\n" +
                "• ios/Pods/, android/.gradle/, android/app/build/, android/build/\n\n" +
                "Your source code is untouched. The next flutter build / flutter run regenerates everything (slower first build).",

            "cache.webFrontend": "Web Frontend Builds",
            "cache.webFrontend.detail":
                "Per-project build outputs of modern web frameworks (Next.js, Nuxt, Vite, Astro, Remix, SvelteKit, Gatsby, Turbo):\n\n" +
                "• .next/, .nuxt/, dist/, out/\n" +
                "• .turbo/, .vite/, .parcel-cache/, .cache/, node_modules/.cache/\n\n" +
                "Your source code is untouched. The dev server / build command regenerates everything on next run.",

            "cache.iosProject": "iOS / CocoaPods Project Builds",
            "cache.iosProject.detail":
                "Build artifacts inside each native iOS project that has a Podfile (excludes RN/Flutter ios/ folders):\n\n" +
                "• Pods/ — installed pod dependencies\n" +
                "• build/ — Xcode build output\n\n" +
                "⚠️ Deleting Pods/ means you must run pod install before building again. Your source code and Podfile are untouched.",

            "cache.androidProject": "Android Gradle Project Builds",
            "cache.androidProject.detail":
                "Build artifacts inside each native Android Gradle project (build.gradle at root, excludes RN/Flutter):\n\n" +
                "• build/, app/build/\n" +
                "• .gradle/ (project-local Gradle cache)\n\n" +
                "Your source code is untouched. Gradle rebuilds everything on the next build (slower first run).",

            "cache.pythonProject": "Python Project Caches",
            "cache.pythonProject.detail":
                "Cache folders inside each Python project (requirements.txt, pyproject.toml, setup.py, Pipfile):\n\n" +
                "• __pycache__/ (found recursively throughout the project)\n" +
                "• .pytest_cache/, .mypy_cache/, .ruff_cache/, .tox/\n" +
                "• build/, dist/, htmlcov/, .coverage\n\n" +
                "Your source code and virtualenv are untouched. Python regenerates .pyc files on the next run.",
            ...
            "subitem.lastBuild":      "Last build",
            "subitem.revealInFinder": "Reveal in Finder",
        ...
        .vi: [
            ...
            "cache.rnBuild": "React Native Android Builds",
            "cache.rnBuild.detail":
                "Tất cả build artifacts Android của mỗi project React Native, gom thành 1 mục:\n\n" +
                "• android/app/build, android/build, android/.gradle\n" +
                "• node_modules/<lib>/android/build (thư viện native như libreanimated.so)\n" +
                "• node_modules/<lib>/android/.cxx (cache CMake/NDK)\n\n" +
                "Mã nguồn không bị đụng. Sau khi xóa, Android sẽ build lại toàn bộ ở lần chạy tiếp theo — lần đầu sẽ chậm hơn.",

            "cache.rustProject": "Rust Project target/",
            "cache.rustProject.detail":
                "Thư mục target/ do Cargo tạo trong mỗi project Rust (Cargo.toml).\n\n" +
                "Thường là cache lớn nhất trên máy dev — vài GB / project là bình thường vì target/ chứa cả debug + release và artifacts compiled của mọi dependency.\n\n" +
                "Mã nguồn không bị đụng. Cargo sẽ compile lại dependencies ở lần build kế (chậm lần đầu, sau đó incremental lại).",

            "cache.flutterProject": "Flutter Project Builds",
            "cache.flutterProject.detail":
                "Tất cả build artifacts trong mỗi Flutter project (pubspec.yaml):\n\n" +
                "• .dart_tool/, build/\n" +
                "• ios/Pods/, android/.gradle/, android/app/build/, android/build/\n\n" +
                "Mã nguồn không bị đụng. Lệnh flutter build / flutter run kế tiếp sẽ tự tạo lại (lần đầu sẽ chậm hơn).",

            "cache.webFrontend": "Web Frontend Builds",
            "cache.webFrontend.detail":
                "Build outputs của các framework web hiện đại (Next.js, Nuxt, Vite, Astro, Remix, SvelteKit, Gatsby, Turbo):\n\n" +
                "• .next/, .nuxt/, dist/, out/\n" +
                "• .turbo/, .vite/, .parcel-cache/, .cache/, node_modules/.cache/\n\n" +
                "Mã nguồn không bị đụng. Dev server / lệnh build sẽ tự tạo lại ở lần chạy kế.",

            "cache.iosProject": "iOS / CocoaPods Project Builds",
            "cache.iosProject.detail":
                "Build artifacts trong mỗi project iOS thuần có Podfile (loại trừ ios/ folder của RN/Flutter):\n\n" +
                "• Pods/ — pod dependencies đã cài\n" +
                "• build/ — output của Xcode build\n\n" +
                "⚠️ Xóa Pods/ thì phải chạy pod install lại trước khi build. Mã nguồn và Podfile không bị đụng.",

            "cache.androidProject": "Android Gradle Project Builds",
            "cache.androidProject.detail":
                "Build artifacts trong mỗi project Android Gradle thuần (có build.gradle ở root, không phải RN/Flutter):\n\n" +
                "• build/, app/build/\n" +
                "• .gradle/ (Gradle cache local của project)\n\n" +
                "Mã nguồn không bị đụng. Gradle sẽ build lại toàn bộ ở lần build kế (chậm lần đầu).",

            "cache.pythonProject": "Python Project Caches",
            "cache.pythonProject.detail":
                "Các thư mục cache trong mỗi Python project (requirements.txt, pyproject.toml, setup.py, Pipfile):\n\n" +
                "• __pycache__/ (tìm đệ quy trong toàn project)\n" +
                "• .pytest_cache/, .mypy_cache/, .ruff_cache/, .tox/\n" +
                "• build/, dist/, htmlcov/, .coverage\n\n" +
                "Mã nguồn và virtualenv không bị đụng. Python sẽ tạo lại .pyc files ở lần chạy kế.",
            ...
            "subitem.lastBuild":     "Build lần cuối",
            "subitem.revealInFinder": "Mở trong Finder",
